...
Go to `css/components/info-card.css`. The `.info-card` class will be your Block class so you don't have to touch it. All other selectors should be replaced by class selectors in BEM style (`.info-card__headline` could be nice for the first one). Don't forget to identify the HTML elements that use these styles and add the new classes to them.

Some general guidelines for using BEM:

- Every styled element inside a component needs a **class**, no type selectors!
- Every class name for a given component hast to start with the BLOCK name (the name of the component, here: `info-card`).
