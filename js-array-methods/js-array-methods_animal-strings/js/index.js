...
const firstAnimalStartingWithLetterP = animalStrings.find((element) => element.startsWith("p"));
...
const anyAnimalEndsWithLetterZ = animalStrings.some((element) => element.endsWith("z"));

const everyAnimalHasMoreThanTwoLetters = animalStrings.every((element) => element.length > 2);
...
//const sumOfAllAnimalCharacters = animalStrings.reduce((total, current) => total + current).length;
// OR
